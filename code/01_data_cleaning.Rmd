...
- The data has 406 variables (the number of columns) and 6,448 observations (the number of rows for each column). Also, please note that the result of the `table() function` which shows that every survey participant participated the full survey. 

```{r}

# Import 

data <- read.dta(make_here("/home/jae/analyzing-racial-lived-experience/raw_data/naas16post-weighted.dta"))
...
### 3.2. Capture the first number group from ordered survey responses 
...
While we inspected how values are coded in the data using `the apply() function`, we found that the questions start wiht `q4_5`, `q5_1`, and `q5_2` contain binary responses.
...
write.csv(scaled, make_here("/home/jae/analyzing-racial-lived-experience/processed_data/cleaned.csv"))
