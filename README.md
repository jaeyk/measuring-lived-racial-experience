...
2. In addition, for R statistical packages, I think it would be nice if there were some printed instructions for changing model parameters. In particular, it would be useful if these packages could inform users regarding the limitations of the default setups and the correct ways to interpret model outcomes. Most R statistical packages assume that users have deep knowledge of the functions they are using and therefore it is assumed that the provided documentation is sufficient. I believe that in most cases this assumption is unwarranted. As R gains more popularity, R users become more diverse in their technical backgrounds. Therefore, nudging users to avoid overconfidence could make a meaningful difference in the ways these packages are applied to data analysis.